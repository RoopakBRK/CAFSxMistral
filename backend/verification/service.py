...
from schemas import ExtractionResult, VerificationResult, IssuerName
from config import config
from verification.sources import TrustedSourceRegistry
from verification.scanner import fetch_page_text
...
class VerificationService:
    ...
    def verify_from_evidence(self, evidence_data: dict) -> ExtractionResult:
        ...
        issuer_str = (evidence_data.get('issuer') or '').lower()
        ...
        return ExtractionResult(
            candidate_name=evidence_data.get('student_name'),
            issuer_name=issuer_enum,
            issuer_org=evidence_data.get('issuer'),
            issuer_url=urls[0] if urls else None,
            certificate_id=cert_ids[0] if cert_ids else None,
            course_name=evidence_data.get('course_name'),
            completion_date=evidence_data.get('completion_date')
        )
